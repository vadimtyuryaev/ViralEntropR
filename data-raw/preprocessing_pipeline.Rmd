---
title: "Preprocessing SARS-CoV-2 Sequence Data for Variant Analysis"
...
output:
  html_document:
    ...
    df_print: paged
vignette: >
  %\VignetteIndexEntry{Preprocessing SARS-CoV-2 Sequence Data for Variant Analysis}
...
> **Reproducibility note:** The full 137,132-sequence dataset (~173 MB) is
> archived on Zenodo (DOI: `10.5281/zenodo.19040165`) and can be downloaded
> programmatically using `download_sarscov2_data()`. The data loading section
> below shows both the download approach and how to read a locally cached copy
> from `data-raw/`.
...
## Download (Zenodo -- run once)

`download_sarscov2_data()` fetches the full dataset from Zenodo and caches it
locally in a persistent user-level directory. Subsequent calls return the
cached path immediately without re-downloading.
...
> cytoplasmic tail). Because NCBI complete sequences all share this length,
> all site indices are directly comparable across sequences without any
> multiple sequence alignment step -- site 501 in one sequence is the same
> structural position as site 501 in any other, which is what makes
> site-wise entropy and Hellinger distance calculations meaningful.
...
- **Site 95 (T95I)** -- located in the N-terminal domain (NTD) of the Spike
  ...
  appears in Delta and Kappa.
  [[Magazine et al., 2022 -- *Viruses*]](https://pmc.ncbi.nlm.nih.gov/articles/PMC8949778/)
...
- **Site 655 (H655Y)** -- located immediately upstream of the furin cleavage
  ...
  defining feature of Gamma (P.1) and Omicron.
  [[Escalera et al., 2022 -- *Cell Host & Microbe*]](https://pmc.ncbi.nlm.nih.gov/articles/PMC8776496/)
...
- **Site 677 (Q677H/P)** -- situated only three residues upstream of the
  ...
  a striking example of convergent evolution under positive selection.
  [[Zeng et al., 2021 -- *mBio*]](https://pmc.ncbi.nlm.nih.gov/articles/PMC8527387/)
...
companion vignette *"Simulation-Based Detection of Emerging Viral Variants
Using Entropy Clustering and Change Point Analysis"* for a complete worked
example.

# Pipeline Summary

```
download_sarscov2_data()          # Step 1: Fetch full dataset from Zenodo (once)
  |
  v
readAAStringSet()                 # Load FASTA into R
  |
  v
...
e.agglo()                         # (see companion simulation vignette)
```
