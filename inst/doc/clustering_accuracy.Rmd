---
title: "Clustering Accuracy Estimation: Separating Wild-Type from Delta Variant Sequences"
...
output:
  html_document:
    ...
    df_print: paged
vignette: >
  %\VignetteIndexEntry{Clustering Accuracy Estimation: Separating Wild-Type from Delta Variant Sequences}
...
- id: Peacock2021
  ...
  - family: Barclay
    given: Wendy S.
  title: The furin cleavage site of SARS-CoV-2 spike protein is a key determinant
    for transmission due to enhanced replication in airway cells
...
- id: Korber2020
  ...
  - family: Gnanakaran
    given: Sandrasegaram
  title: "Tracking Changes in SARS-CoV-2 Spike: Evidence that D614G Increases
...
- id: Wrobel2020
  ...
  container-title: Nature Structural and Molecular Biology
  volume: 27
  page: 763-767
  issued:
    year: 2020
  DOI: 10.1038/s41594-020-0468-x

- id: CDC2022
  type: article-journal
  author:
  - literal: CDC COVID-19 Response Team
  title: "Effectiveness of COVID-19 mRNA Vaccines Against the Delta (B.1.617.2)
    Variant Among Adults Aged 18-64 Years --- United States, April 4--July 17, 2021"
  ...
  URL: https://www.cdc.gov/mmwr/volumes/71/wr/mm7106a4.htm
---
...
clarity. A **variant of concern (VOC)** is a SARS-CoV-2 lineage designated by
the WHO as carrying mutations with demonstrated impact on transmissibility,
disease severity, or immune evasion. A **variant of interest (VOI)** is a
lineage under active monitoring for potentially similar properties but without
yet sufficient evidence for VOC designation. A **single nucleotide polymorphism (SNP)**, 
used here in the amino acid sense, refers to a specific substitution
at a defined Spike protein site that is characteristic of a given lineage and
used as a defining marker in surveillance catalogues. A **Gaussian mixture model (GMM)** 
is a probabilistic model that represents a given distribution as a
weighted sum of Gaussian components; here it is applied to the vector of
per-site Shannon entropies to identify sites with unusually high variability
--- those sites form the discriminating feature set passed to the clustering
step.
...
the two time windows, and `K_HIGHLIGHT` before re-rendering; no other chunk needs to change.
...
> already under strong positive selection pressure.
>
> Key references: [@Peacock2021;@Johnson2021]
...
to a 2D layout by preserving local neighbourhood structure [@vanderMaaten2008].
...
to ensure consistency between the clustering and its visualisation.

To reiterate, t-SNE is run once on the Gower distance matrix and the resulting 
2D coordinatesare reused for all k values. This ensures that spatial positions
are identicalacross plots so that differences in cluster boundaries are directly 
comparable.
...
The full-dataset analysis (Step 9) is conceptually distinct from the accuracy
study. It operates on all ~109,536 available USA sequences across the full
surveillance timeline, with GMM site selection (`selected_sites_full`) performed
on the entire dataset. The selected sites therefore reflect population-wide
entropy rather than the contrast between two specific windows, and the two site
sets are expected to differ --- this is by design.
...
efficient framework for detecting and characterising viral variant structure
in large sequence datasets, combining rigorous information theory with the
interpretability required for epidemiological application.
